...
def _generate_c(entries: list[Entry], compressed: bytes, uncompressed_size: int) -> str:
    ...
    lines.append("        gUncompressedData = (unsigned char *)malloc(1);")
    lines.append("        if (gUncompressedData == NULL) {")
    ...
    lines.append("")
    lines.append("        gUncompressedData = (unsigned char *)malloc(gUncompressedDataSize);")
    lines.append("        if (gUncompressedData == NULL) {")
    lines.append("            return -1;")
    lines.append("        }")
    lines.append("")
    ...
    lines.append("            free(gUncompressedData);")
